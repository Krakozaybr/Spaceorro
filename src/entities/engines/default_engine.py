...
class DefaultEngine(Engine):
    def __init__(
        self, body: pymunk.Body, control_body: pymunk.Body, max_speed=inf, stop_coef=1.0, stop_rotation_coef=0.1
    ):
        self.rotation_speed = 100
        self.direct_force = 100
        self.body = body
    ...
    def apply_force(self, force: Vec2d, dt: float):
        self.body.velocity += force.normalized() * self.direct_force * dt
        normalized, length = self.body.velocity.normalized_and_length()
        if self.max_speed < length:
            self.body.velocity = self.max_speed * normalized
